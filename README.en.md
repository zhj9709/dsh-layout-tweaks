...
- **Cache hit with two decimals (pills, default off)** — shows the new stats pills' cache-hit share with two decimal places (e.g. `87.35%`) instead of integer rounding; the token-usage dialog follows. Hidden in Settings while the legacy stats line is on (that line replaces the pills outright); with both toggles on, the legacy line wins.
...
style-tweaks:
  ...
  pillsCacheHitDecimals: false     # default false; true shows the pills' cache hit with two decimals
```
