...
- **缓存命中两位小数（胶囊，默认关闭）**：新版统计胶囊的缓存命中率按两位小数显示（如 87.35%），不再取整；点开的 Token 用量弹窗同步生效。经典统计行开启时本项在设置面板中隐藏（该行已整体替换胶囊）；两个开关同时为开时由经典统计行生效。
...
style-tweaks:
  ...
  pillsCacheHitDecimals: false  # 默认 false；true 则新版胶囊的缓存命中率显示两位小数
```
